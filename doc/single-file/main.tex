...
Celem pracy jest zaprojektowanie i zaimplementowanie aplikacji webowej, która będzie pełnić funkcję inteligentnego kreatora zestawów komputerowych. System będzie dawać użytkownikowi możliwosć wybrania dowolnych produktów z katalogu, sprawdzenie czy aktualne zamówienie jest wzajemnie kompatybilne, a gdy weryfikacja zakończy się pomyślnie, umożliwnie realizacji zamównienia. Idące za tym cele to
\begin{itemize} 
...
\begin{itemize}
	...
	\item Utworzenie serwera PostgreSQL, wprowadzenie gotowych danych o produktach do bazy danych. \item Stworzenie użytkowników oraz nadanie odpowiednich uprawnień każdemu z nich.
...
Autor samodzielnie opracował wszystkie elementy projektu, zaczynając od zaplanowania funkcjonalności oraz wyborze technologii. Następnym krokiem było zaprojektowanie relacyjnej bazy danych w Oracle SQL Developer, utworzenie serwera PostgreSQL wraz z użytkownikami oraz nadanie odpowiednich uprawnień każdemu z nich. Przy użyciu frameworku FastAPI przygotowany został backend w języku Python, odpowiadający za komunikację z bazą danych oraz za logikę całej aplikacji, umożliwiając weryfikację kompatybilności zestawów komputerowych. Finalnie autor utworzył interaktywny frontend, który umożliwia graficzne korzystanie z aplikacji.
...
\subsection{Niska dostępność do weryfikacji kompatybilności}

Aktualnie na polskim rynku sklepów komputerowych brakuje narzędzi do samodzielnej weryfikacji kompatybilności wybranych części. Większość sprzedawców oferuje swoje gotowe zestawy, które zapewniają pełną funkcjonalność, jednak często nie posiadają one możiwości rozbudowania stanowiska o nowe komponenty, jak również wymiany już zamontowanych elementów na nowsze modele. Zaprojektowanie stanowiska w pełni odpowiadającego potrzebom klienta jest wówczas stanowczo utrudnione i może potencjalnie wymagać kontaktu ze specjalistą, co bezpośrednio przekłada się na dodatkowe koszty oraz wydłużenie czasu realizacji zamówienia.

\section{Analiza istniejących rozwiązań w najpopularniejszych sklepach na polskim rynku}
\subsection{Morele.net}
Po wnikliwej analizie dostępnych platform sprzedażowych polskich sprzedawców, najlepiej zaprojektowanym rozwiązaniem okazał się konfigurator zestawów PC sklepu Morele. Wyposażony w intuicyjny interfejs, umożliwia on wybranie wszystkich dostępnych komponentów, jednak kluczowym problemem jest brak możliwości weryfikacji kompatybilności w sposób automatyczny, w przypadku błędnie zaprojektowanego zestawu komputerowego, użytkownik może liczyć na ewentualny kontakt ze strony pracownika, co w praktyce znacząco opóźnia realizację zamówienia i realnie zagraża realizacji zamówienia, które zawiera wzajemnie niepasujące produkty.
\subsection{X-kom.pl}
\subsection{Komputronik.pl}
\section{Analiza istniejących rozwiązań w najpopularniejszych sklepach na zagranicznym rynku}
